...
| `v1.8.12` | Fixed real-time sync getting permanently stuck on one device when editing from two at once: the tab that had typed most recently (and still had the editor focused, even idle) silently absorbed every future remote update into its local version without ever applying the content, so it never caught up until a manual reload. Also added a 👁 badge that shows when someone else currently has the same pad open (via the existing WebSocket presence channel), hidden when you're the only one there. Desktop app binaries unchanged from `v1.8.6` |
| `v1.8.13` | Two follow-ups to `v1.8.12`: remote edits now apply live in the editor the instant they arrive, even while it's focused — only actively typing (last 4s) still defers them, instead of any focus at all requiring a blur first. And the 👁 viewer badge now disappears faster when someone else's connection drops uncleanly (network loss, sleep, etc.) — WebSocket ping/timeout tightened from 30s/60s to 8s/20s; a clean tab close was already near-instant. Desktop app binaries unchanged from `v1.8.6` |
| `v1.8.14` | Fixed the 👁 viewer badge taking 20+ seconds to disappear when someone switched to a different pad (via the "Go" field) instead of closing the tab — the browser doesn't always tear down an in-flight WebSocket promptly on that kind of navigation, so the server only noticed once the connection timed out. Now closed explicitly on `pagehide`, so it's detected immediately, same as a closed tab. Desktop app binaries unchanged from `v1.8.6` |
| `v1.8.15` | Tightened the WebSocket ping/read-deadline further, from 8s/20s to 4s/10s — halves the worst case for the 👁 badge to notice a connection that died without warning (network loss, sleep, app killed), now that clean navigation and tab-close are already instant via `v1.8.14`'s `pagehide` fix. Kept at 2 missed pings' worth of tolerance to avoid the badge flickering on brief network blips. Desktop app binaries unchanged from `v1.8.6` |
| `v1.8.16` | Pasting a table from Confluence, Notion, or Excel into a pad now renders as a real HTML table instead of losing its structure — the editor is a rich (contenteditable) area rather than a plain textarea, so copying it back out also produces real `<table>` markup another Confluence/Notion page can read in. Pasted HTML is sanitized to a narrow allowlist both client-side and server-side (in both servers) before it's stored and synced to other live viewers. Existing plain-text pads keep rendering exactly as before. Server-only change — desktop app binaries unchanged from `v1.8.6` |
| `v1.8.17` | Three follow-ups to `v1.8.16`. **Fixed a regression**: pasting a tall table (or any long content) into the editor pushed the whole page taller and left the footer stranded partway down instead of at the bottom, because `<main>` was missing `min-height:0` — a `<textarea>` never hit this, but the new contenteditable `#editor`'s content does; it now scrolls internally, like a textarea did, and the page stays at viewport height. Also made the footer's hint text unselectable — it's decorative, not meant to be copied. **Fixed pasting from Excel** rendering as an uploaded image instead of a table: Excel (and Word, Sheets, ...) put both an HTML table and a bitmap of the same selection on the clipboard, and the paste handler checked for files first, silently uploading the image and never looking at the table. **Dropping a folder onto a pad now zips it client-side** (via `fflate`, vendored locally like `DOMPurify`) and uploads the result, instead of failing — bounded by a file-count/total-size cap so a huge tree can't hang the browser tab zipping it, on top of the server's existing upload size limit on the finished zip. Server-only change — desktop app binaries unchanged from `v1.8.6` |
